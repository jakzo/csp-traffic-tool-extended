...
function TrafficLight:update(cameraPosition, dt)
  ...
  if updateEmissive and not self.emissive then
    self.emissive = self._intersection._sides:mapTable(function(side, _, data_)
      local i = side.index
      return i,
          data_.emissive and data_.emissive[i] and (data_.emissive[i].mode or data_.emissive[i].roles) and
          TrafficLightEmissive(side, data_.emissive[i]) or nil
    end, self._data)
...
function TrafficLight:draw3D(layers)
  for i = 1, self.sides.length do
    local side = self.sides[i]
    local pos = (side.entries[#side.entries].fromPos - side.centerEntries):normalize():add(side.entries[#side.entries]
    .fromPos):add(vec3(0, 3, 0))
    local tlState = side.tlState
    renderQuad(pos, -side.entryDir,
      tlState == IntersectionLink.StateGreen and _colors.greenActive or _colors.greenInactive)
    renderQuad(pos:add(vec3(0, 0.5, 0)), -side.entryDir,
      (tlState == IntersectionLink.StateYellow or tlState == IntersectionLink.StateRedYellow) and _colors.yellowActive or
      _colors.yellowInactive)
    renderQuad(pos:add(vec3(0, 0.5, 0)), -side.entryDir,
      (tlState == IntersectionLink.StateRed or tlState == IntersectionLink.StateRedYellow) and _colors.redActive or
      _colors.redInactive)
  end
end
